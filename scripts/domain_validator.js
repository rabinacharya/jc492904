...
$(document).ready(function(e) {
		...
		//Checking Validity of name when key is pressed
		$("#name").keyup(function(){
		...
		$("#reg_button").click(function(){
			...
			//checking name	
			  ...
			  {
			      $('#icon_image_name').attr("src","assets/Accept-icon.png");
				  $("#name_help").hide(1200);
			  }
			  else
			  {
			      $('#icon_image_name').attr("src","assets/Close-icon.png");
				  $("#name_help").show("slow");
				  flag=-1
		...
		//Checking all select box is emptied or not qualificatioN
			   ...
			   {
				   $('#icon_image_Qualification').attr("src","assets/Accept-icon.png");
				   $("#Qualification_help").hide(1200);
			   }  
			   else
			   {
				   $('#Qualification_'+this.id).attr("src","assets/Close-icon.png");
				   $("#Qualification_help").show("slow");
				   flag=-1
			   }
		
		//Checking EmailID
		  if($("#email").val().match(/^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+[.][A-Z-a-z]{2,4}$/))
		  {
		    	$('#icon_image_email').attr("src","assets/Accept-icon.png");
				$("#email_help").hide(1200);
		  }
		  else
		  {
                $('#icon_image_email').attr("src","assets/Close-icon.png");
				$("#email_help").show("slow");
				flag=-1
 		   }
		...
		//Checking Username
		  ...
		  else
		  {
		   ...
		   if($("#pwd").val().length>5)
		   {
			  $('#icon_image_pwd').attr("src","assets/Accept-icon.png");
			  $("#pwd_help").hide(1200);   
		   }
		   else
		   {
			  $('#icon_image_pwd').attr("src","assets/Close-icon.png");
			  $("#pwd_help").show("slow");
			  flag=-1
		   }
		...
		//Checking Password Match
		  ...
		  {
			  $('#icon_image_cnf_pwd').attr("src","assets/Accept-icon.png");
			  $("#cnf_pwd_help").hide(1200);
		  }
		  else
		  {
			  $('#icon_image_cnf_pwd').attr("src","assets/Close-icon.png");
			  $("#cnf_pwd_help").show("slow");
			  flag=-1
		  }
		
		//Checking Mobile Number
			if($("#phn").val().match(/^[0-9]{10}$/))
			{
			    $('#icon_image_phn').attr("src","assets/Accept-icon.png");
			    $("#phn_help").hide(1200);
				}
			else
			{
				$('#icon_image_phn').attr("src","assets/Close-icon.png");
			    $("#phn_help").show("slow");
				flag=-1
			}	
